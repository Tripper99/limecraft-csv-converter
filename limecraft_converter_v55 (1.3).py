...
TIME_THRESHOLD = 0.01
VERSION = "55"
REQUIRED_COLUMNS = ['Media Start', 'Transcript']  # Only keep columns we actually use
...
class LimecraftConverter:
    ...
    def adjust_timecodes(self, start_time_str: str) -> None:
        ...
        try:
            ...
            for row in self.data:
                current_seconds = self.parse_timecode(row['Media Start'])
    ...
    def add_starting_timecode(self, time_adjustment_applied: bool = False) -> None:
        ...
        if not math.isclose(first_seconds, 0.0, abs_tol=TIME_THRESHOLD):
            ...
            starting_entry = {
                ...
                'Transcript': transcript_text  # No Speakers column needed
            }
    ...
    def _validate_and_normalize_columns(self, data: List[Dict[str, str]]) -> List[Dict[str, str]]:
        ...
        # Keep ONLY the required columns, discard everything else (including Speakers, Media Duration, etc.)
        normalized_data = []
        for row in data:
            normalized_row = {}
            for standard_name, actual_name in column_mapping.items():
                normalized_row[standard_name] = row.get(actual_name, '')
    ...
    def convert_to_word(self, output_path: str, filename: str, include_filename_prefix: bool = False) -> bool:
        ...
        try:
            ...
            for i, row in enumerate(self.data):
                ...
                # Add transcript text if it exists
                if transcript:
                    text_para = doc.add_paragraph(transcript)
...
1. Välj CSV-fil (som du exporterats från Limecraft)
   ...
   Rutorna framför "Media Start" och "Transcript" måste vara ikryssade. Det är ok om fler rutor är ikryssade men ingen annan metadata används.)
    ...
    def validate_inputs(self) -> bool:
        ...
        if self.start_time_var.get().strip():
            try:
                ...
                normalized_time = self.converter.normalize_time_input(self.start_time_var.get().strip())
